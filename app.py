...
# Asumo que las funciones de 'processing' están en un archivo separado
...
# Si 'processing.py' no existe, las funciones deberían estar en este mismo archivo
# para que el código sea autocontenido. Aquí las incluyo como placeholders.
def apply_filters(data, srate, highpass, lowpass, notch):
    # Lógica de filtros (ejemplo)
    if highpass:
        sos = signal.butter(4, highpass, 'hp', fs=srate, output='sos')
        data = signal.sosfilt(sos, data)
    if lowpass:
        sos = signal.butter(4, lowpass, 'lp', fs=srate, output='sos')
        data = signal.sosfilt(sos, data)
    if notch:
        b, a = signal.iirnotch(notch, Q=30, fs=srate)
        data = signal.filtfilt(b, a, data)
    return data

def spectral_analysis(data, srate):
    freqs, psd = signal.welch(data, fs=srate, nperseg=2048)
    return {'freqs': freqs, 'psd': psd}

def detect_markers(signal, srate, threshold, time_after, time_before, after_a, before_a, duration):
    # Lógica de detección de marcadores (ejemplo)
    rectified_signal = np.abs(signal)
    max_val = np.max(rectified_signal)
    min_val = np.min(rectified_signal)
    if max_val - min_val == 0:
        return []
    scaled_signal = (rectified_signal - min_val) / (max_val - min_val)

    threshold_crossings = np.where(np.diff((scaled_signal > threshold).astype(int)) == 1)[0]

    valid_markers = []

    time_after_samples = int(time_after * srate)
    time_before_samples = int(time_before * srate)
    duration_samples = int(duration * srate)

    for crossing in threshold_crossings:
        # Validación de amplitud antes y después
        if crossing - time_before_samples >= 0 and crossing + time_after_samples < len(scaled_signal):
            if np.mean(scaled_signal[crossing - time_before_samples:crossing]) < before_a and \
               np.mean(scaled_signal[crossing:crossing + time_after_samples]) > after_a:
                # Validación de duración mínima (evita detección de bursts muy cortos y consecutivos)
                if not valid_markers or (crossing - valid_markers[-1]) > duration_samples:
                    valid_markers.append(crossing)

    return np.array(valid_markers)
...
if uploaded_file is not None:
    ...
    # PESTAÑAS: Visualización de Señales, Análisis Espectral y Detección de Marcadores
    tab1, tab2, tab3 = st.tabs(["Visualización de Señales", "Análisis Espectral", "Detección de Marcadores"])
            ...
            if st.button("🔍 Detectar Marcadores EMG"):
                ...
                if len(markers) > 0:
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name=f"marcadores_{uploaded_file_name.split('.')[0]}.csv",
                        mime="text/csv",
                        key = "dwbt2"
                    )
                    ...
                    if 'timeseries_export' in st.session_state:
                        ...
                        st.download_button(
                            ...
                            data=timeseries_csv,
                            file_name=f"serie_tiempo_emg_{uploaded_file_name.split('.')[0]}.csv",
                            mime="text/csv",
                            ...
                            if st.button("📋 Generar Segmentos de Marcadores", key = "markers-segments"):
                                ...
                                if segments_data:
                                    ...
                                    st.download_button(
                                        ...
                                        data=segments_csv,
                                        file_name=f"segmentos_marcadores_{uploaded_file_name.split('.')[0]}.csv",
                                        mime="text/csv",
                                        key = "dwb1"
                                    )
        else:
            st.warning("Por favor, carga un archivo y configura los canales en la pestaña 'Visualización de Señales' para realizar el análisis de detección.")
